...
# --- HEDEF VE EKSİK YARDIMCI FONKSİYONLARI ---
...
# --- TAM MENÜ LİSTESİ (ADMİN İÇİN TÜM HAKLAR GERİ GETİRİLDİ) ---
...
if secim == "📊 Genel Bakış":
    ...
    df_last_exams = pd.read_sql_query("SELECT sinav_adi, tarih, sinav_turu FROM sinavlar ORDER BY tarih DESC LIMIT 5", conn)
...
# --- 4. SINAV YÖNETİMİ ---
    ...
    conn = sqlite3.connect("sinav_takip.db")
    df_sinavlar = pd.read_sql_query("SELECT sinav_id, sinav_adi, tarih, sinav_turu FROM sinavlar ORDER BY tarih DESC", conn)
...
# --- 6. HEDEF YÖNETİMİ ---
    ...
    if not df_ogr.empty:
        ...
        if st.button("Target Kaydet / Güncelle", type="primary"):
            cursor = conn.cursor()
            cursor.execute('''
        ...
        st.info("Kayıtlı öğrenci verisi yok.")
    conn.close()

# --- 7. KURUM AYARLARI ---
elif secim == "⚙️ Kurum Ayarları" and st.session_state['role'] == 'admin':
    st.title("⚙️ Kurum Ayarları")
    st.info("Kurum adı ve Logo özelleştirme ayarlarını buradan yapılandırabilirsiniz.")