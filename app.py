...
def tr_clean(text):
    if not isinstance(text, str):
        return text
    replacements = {'I': 'I', 'ı': 'i', 'İ': 'İ', 'Ş': 'S', 'ş': 's', 'Ğ': 'G', 'ğ': 'g', 'Ü': 'U', 'ü': 'u', 'Ö': 'O', 'ö': 'o'}
...
# --- YARDIMCI MOTOR DİĞER FONKSİYONLAR ---
...
def generate_pdf_report(ogr_adi, hedef, df_sonuc, eksik_listesi, halledilen_listesi, ai_plan, notlar):
    ...
    p.drawString(100, 750, f"SINAV GELISIM KARNESI - {tr_clean(ogr_adi).upper()}")
    p.drawString(100, 730, "-" * 50)
    
    y = 700
    if hedef:
        p.drawString(100, y, f"Hedef: {hedef['bolum']} | Net: {hedef['net']} | Puan: {hedef['puan']}")
        y -= 20
        
    p.drawString(100, y, f"Toplam Katilinan Sinav Sayisi: {len(df_sonuc)}")
    y -= 30
    
    p.drawString(100, y, "Aktif Eksik Konular:")
    y -= 15
    for item in eksik_listesi[:5]:
        p.drawString(120, y, f"- {item['ders']}: {item['konu']}")
        y -= 15
