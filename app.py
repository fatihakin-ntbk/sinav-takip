...
def generate_pdf_report(ogr_adi, hedef, df_sonuclari, eksik_konular, halledilen_konular, ai_plan, notlar):
    ...
    if not df_sonuclari.empty:
        ...
        if 'tyt_toplam' in df_sonuclari.columns:
            ax.plot(df_sonuclari['sinav_adi'], df_sonuclari['tyt_toplam'], marker='o', color='#2B6CB0', linewidth=2, label='TYT Net')
        if 'ayt_toplam' in df_sonuclari.columns and (df_sonuclari['ayt_toplam'] > 0).any():
            ax.plot(df_sonuclari['sinav_adi'], df_sonuclari['ayt_toplam'], marker='s', color='#E53E3E', linewidth=2, label='AYT Net')
            
        ax.grid(True, linestyle='--', alpha=0.5)
        ax.set_ylim(0, 120)
        plt.xticks(rotation=8, fontsize=7)
